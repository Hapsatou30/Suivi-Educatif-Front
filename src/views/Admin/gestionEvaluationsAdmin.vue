    ...
    <div class="classes">
      ...
      <div class="tableau1">
        <tabEvaluations v-if="paginatedData.length > 0" class="tab-gestionNotes"
          :headers="['N°', 'Nom ', 'Capacité', 'Niveau', 'Évaluations']" :data="paginatedData.map(({ numero, nom, capacite, niveau, id, anneeClasse_id }) => ({
            numero,
            nom,
            capacite,
            niveau,
...
const redirectToSubeject = (anneeClasse_id) => {
  ...
  router.push({ name: 'evaluations_classe', params: { anneeClasse_id } });
};
