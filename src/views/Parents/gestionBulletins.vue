<template>
    ...
    <div class="main-content1 ">
        <h1>Voir les bulletins de mes enfants</h1>
        <div class="row">
            <div class="col-lg-4 col-md-6 col-12" v-for="card in cards" :key="card.id">
                <!-- Envelopper la carte dans un lien -->
                <router-link :to="{ name: 'bulletins_par_enfant_1semestre', params: { classeEleve_id: card.classeEleve_id } }" class="text-decoration-none">
    ...
    try {
        ...
        if (response.status === 200 && response.données) {
            cards.value = response.données.map((eleve, index) => ({
                ...
                photo: eleve.photo ? `SuiviEducatifFrontEnd.thiamhapstou.simplonfabriques.com/storage/${eleve.photo}` : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZv5fMEw3s3nvP0sxLIG8bO6RzCLmqgzW5ww&s',
