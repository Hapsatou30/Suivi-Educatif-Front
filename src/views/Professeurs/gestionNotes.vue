    ...
    <div class="classes">
      <div class="tableau1">
        <tabEvaluations v-if="paginatedData.length > 0" class="tab-classes"
          :headers="['Nom de la classe ', 'Matières', 'Nombre Eleve', 'Action']" :data="paginatedData.map(({ nom_classe, nom_matiere, capacite, classeProf_id, annee_classe_id }) => ({
            nom_classe,
            nom_matiere,
            capacite,
            classeProf_id,
          ...
          <template #actions="{ row }">
            <div class="boutons">
              <button class="btn" @click="seeClasse(row.classeProf_id, row.annee_classe_id, row.nom_classe)"
                style="color: red; font-size: 40px;" title="Voir les horaires de cette classe">
