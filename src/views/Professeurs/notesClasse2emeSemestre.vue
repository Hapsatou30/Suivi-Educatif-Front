...
const fetchData = async () => {
    try {
        ...
        // Filtrer les données pour ne garder que celles avec la période '2_semestre'
        const filteredData = response.données.filter(note => note.bulletin_id.periode === '2_semestre');
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            const eleves = response.données[0].eleves.map(eleve => {
                // Filtrer les bulletins pour garder uniquement ceux avec la période "2_semestre"
                const bulletinsFiltrés = eleve.bulletins.filter(bulletin => bulletin.periode === "2_semestre");
            ...
            // console.log('Données des élèves avec bulletins 2_semestre:', notesStudents.value);
